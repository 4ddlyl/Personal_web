...
<body class="text-white font-sans min-h-screen relative">
  ...
  <header class="text-white py-6 shadow-lg flex justify-between items-center px-4 md:px-10">
    <h1 class="text-2xl md:text-3xl font-bold">// Edit Data About //</h1>
  ...
  <div class="flex flex-col md:flex-row max-w-7xl mx-auto mt-8 px-4">
    ...
    <!-- Main Content -->
    <main class="glass p-6 w-full md:w-3/4 mt-6 md:mt-0 md:ml-6">
      <form action="proses_edit_about.php" method="post" class="space-y-6">
        ...
        <div>
          <label for="about" class="block text-sm font-medium text-white mb-1">Tentang Saya</label>
          <textarea id="about" name="about" rows="5" required
            class="w-full p-3 bg-white bg-opacity-10 border border-white border-opacity-20 rounded focus:outline-none focus:ring-2 focus:ring-purple-500 text-white placeholder-white"><?php echo htmlspecialchars($data['about']); ?></textarea>
        </div>
        <div class="flex justify-end space-x-4">
          <button type="submit"
            class="bg-purple-600 text-white px-4 py-2 rounded hover:bg-purple-700 transition">Update</button>
          <a href="about.php"
