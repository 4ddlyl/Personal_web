...
<body class="text-white font-sans min-h-screen relative">
  ...
  <header class="text-white py-6 shadow-lg flex justify-between items-center px-4 md:px-10">
    <h1 class="text-2xl md:text-3xl font-bold">// Kelola About //</h1>
  ...
  <div class="flex flex-col md:flex-row max-w-7xl mx-auto mt-8 px-4">
    ...
    <main class="w-full md:w-3/4 glass p-6 mt-6 md:mt-0 md:ml-6">
      <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-bold text-white drop-shadow-md">Tentang Saya</h2>
        <a href="add_about.php"
          class="bg-purple-600 text-white px-4 py-2 rounded hover:bg-purple-700 transition">+ Tambah Data</a>
      </div>
      <div class="space-y-4">
        <?php
        ...
        while ($data = mysqli_fetch_array($query)) {
          echo "<div class='p-4 bg-white bg-opacity-10 border border-white border-opacity-30 rounded-lg shadow-md text-white'>";
          echo "<p class='mb-3'>" . htmlspecialchars($data['about']) . "</p>";
          echo "<div class='flex space-x-4 text-sm'>";
          echo "<a href='edit_about.php?id_about={$data['id_about']}' class='text-blue-400 hover:underline'>Edit</a>";
          echo "<a href='delete_about.php?id_about={$data['id_about']}' onclick='return confirm(\"Yakin ingin menghapus?\")' class='text-red-400 hover:underline'>Hapus</a>";
          echo "</div></div>";
        }
        ?>
      </div>
