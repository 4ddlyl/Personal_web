...
<body class="text-white font-sans min-h-screen relative">
  ...
  <header class="text-white py-6 shadow-lg flex justify-between items-center px-4 md:px-10">
    <h1 class="text-2xl md:text-3xl font-bold">// Edit Gambar //</h1>
      ...
      <form action="proses_edit_gallery.php" method="post" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
          ...
          <input type="text" id="judul" name="judul" required value="<?php echo htmlspecialchars($data['judul']); ?>"
            class="w-full p-2 rounded bg-white bg-opacity-20 border border-white text-white focus:outline-none focus:ring focus:border-purple-400 placeholder-white">
        </div>
        <div>
        ...
        <div>
          <label for="foto" class="block text-sm font-medium mb-1">Ganti Gambar (Opsional)</label>
          <input type="file" id="foto" name="foto" accept="image/*"
            class="block w-full text-sm text-white file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-purple-100 file:text-purple-700 hover:file:bg-purple-200">
        </div>
        <div class="flex justify-end space-x-4">
          <button type="submit"
            class="bg-purple-700 text-white px-4 py-2 rounded hover:bg-purple-800 transition">Simpan Perubahan</button>
          <a href="data_gallery.php"
            class="bg-gray-300 text-gray-900 px-4 py-2 rounded hover:bg-gray-400 transition">Batal</a>
        </div>
      </form>
    </main>
  </div>
