...
<head>
  <meta charset="UTF-8">
  <title>Tambah Gambar ke Gallery</title>
...
<body class="text-white font-sans min-h-screen relative">
  ...
  <header class="text-white py-6 shadow-lg flex justify-between items-center px-4 md:px-10">
    <h1 class="text-2xl md:text-3xl font-bold">// Tambah Gambar //</h1>
  ...
  <div class="flex flex-col md:flex-row max-w-7xl mx-auto mt-8 px-4">
    ...
    <main class="w-full md:w-3/4 glass p-6 mt-6 md:mt-0 md:ml-6">
      <form action="proses_add_gallery.php" method="post" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
          <label for="judul" class="block text-sm font-medium text-white mb-1">Judul Gambar</label>
          <input type="text" id="judul" name="judul" required
            class="w-full p-2 rounded bg-white/10 border border-white/30 text-white focus:outline-none focus:ring focus:ring-purple-500 focus:border-purple-500">
        </div>

        <div>
          <label for="foto" class="block text-sm font-medium text-white mb-1">Pilih Gambar</label>
          <input type="file" id="foto" name="foto" accept="image/*" required
            class="block w-full text-sm text-gray-300 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-purple-100 file:text-purple-700 hover:file:bg-purple-200">
        </div>
        ...
        <div class="flex justify-end space-x-4">
          ...
          <a href="data_gallery.php"
            class="bg-gray-300 text-gray-800 px-4 py-2 rounded hover:bg-gray-400 transition transform hover:scale-105">Batal</a>
        </div>
      </form>
    </main>
  </div>
